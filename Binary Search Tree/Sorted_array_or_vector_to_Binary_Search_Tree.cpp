...
node* sorted_array_to_bst(vector<int>,int,int);
...
int main()
    {
        ...
        node* ans = sorted_array_to_bst(in,0,in.size()-1);
        preorder(ans);
...
node* sorted_array_to_bst(vector<int> v,int s,int e)
    {
        if(s>e)
            {
                return NULL;
        ...
        root->left = sortedarraytobt(v,s,mid-1);
        root->right = sortedarraytobt(v,mid+1,e); 
